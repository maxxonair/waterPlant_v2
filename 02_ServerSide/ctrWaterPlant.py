...
SCHEDULE_INPUT_PATH = 'wateringTimes.json'
...
def checkForcedUpdate():
    global wateringJson
    # Read data from json 
    with open(SCHEDULE_INPUT_PATH, 'r') as openfile:     
        wateringJson = json.load(openfile)
        
    settings = wateringJson['Settings']
    if settings['force_update'] == 1 :
        # Acknowledge forced update and write back 0
        settings['force_update'] = 0
        wateringJson['Settings'] = settings
        writeWateringJson()
...
def writeWateringJson():
    with open(SCHEDULE_INPUT_PATH, 'w', encoding='utf-8') as f:
        json.dump(wateringJson, f, ensure_ascii=False, indent=4)
        
def readWateringTimes():
    global wateringJson
    global wateringSchedule
    global pumpTimes
    global wateringPunchCard
    ...
    wateringSchedule = []
    pumpTimes = []
    command_set = wateringJson['waterSets']
    for i, pump in enumerate( command_set ):
        wateringSchedule.append( pump['watering_schedule'] )
        pumpTimes.append( pump['pump_cmd_times'] )
        ...
        for j, timeslot in enumerate( pump ):
            if ( isTime > timeslot and wateringPunchCard[i][j] == 0 ):
                ...
                if ( i == 0 ):
                    ...
                    client.publish(MQTT_TOPIC_TC, cmd) 
                    time.sleep(0.8)
                elif ( i == 1 ):
                    # pump 02 
